        ...
        {Array.isArray(items) && items.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 md:gap-10">
            {items.map((item) => {
              ...
              return (
                <div key={id} className="relative flex flex-col overflow-hidden rounded-2xl bg-card border border-border/50 shadow-sm transition-all duration-500 hover:shadow-2xl hover:shadow-brand-500/10 hover:-translate-y-2 group cursor-default select-none pointer-events-none md:pointer-events-auto">
                  <div className="relative h-full flex flex-col">
                    ...
                    <div className="relative z-10 flex flex-col h-full min-h-[420px] p-8 mt-auto justify-end">
                      {richText && (
                        <div className="relative transition-all duration-500 ease-in-out md:group-hover:-translate-y-1">
                          <RichText
                            data={richText}
                            enableGutter={false}
                            className="text-white [&_*]:text-white [&_h3]:type-headline-4 [&_h3]:mb-3 [&_h4]:type-title-lg [&_h4]:mb-2 [&_p]:type-body-md [&_p]:leading-relaxed [&_p]:line-clamp-none md:[&_p]:line-clamp-2 group-hover:[&_p]:line-clamp-none transition-all duration-500"
                          />
                        </div>
                      )}
