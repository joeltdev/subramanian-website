      ...
      <section>
        <div className={`relative py-24 ${hasBgMedia ? 'min-h-[85svh] flex flex-col justify-end md:justify-center pb-20 md:pb-24' : ''}`}>
          ...
          <>
            ...
            {backgroundVideo && typeof backgroundVideo === 'object' && backgroundVideo.url && (
              <video
                className={`absolute inset-0 size-full object-cover object-top md:object-center -z-20 ${mobileBackgroundVideo || mobileBackgroundImage ? 'hidden md:block' : 'block'}`}
            ...
            {backgroundImage && typeof backgroundImage === 'object' && (backgroundImage as Media).url && (
              <img
                className={`absolute inset-0 size-full object-cover object-top md:object-center -z-20 ${mobileBackgroundVideo || mobileBackgroundImage ? 'hidden md:block' : 'block'} ${backgroundVideo ? 'opacity-0' : 'opacity-100'}`}
                ...
                <AnimatedGroup
                  ...
                  {links.map(({ link }, i) => (
                    <CMSLink
                      ...
                      {...link}
                      size="default"
                      className="group w-full sm:w-auto shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 active:scale-[0.98] md:size-xl max-md:bg-brand-100 max-md:text-brand-700 max-md:hover:bg-brand-500 max-md:hover:text-white max-md:border-none"
                    >
