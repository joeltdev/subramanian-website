...
const socialIcons: Record<string, React.ReactNode> = {
  twitter: (
    <svg className="size-[18px] md:size-[26px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
      <path
        fill="currentColor"
...
export async function Footer() {
  ...
  const { socialLinks, copyright, logo: footerLogo } = footerData as Footer
            ...
            <Link href="/" aria-label="go home" className="block size-fit">
              {typeof logo === 'object' && logo && (logo as Media).url ? (
                <img
                  ...
                  loading="lazy"
                  decoding="async"
                  className="h-8 md:h-12 w-auto transition-opacity invert"
                />
              ) : (
                <Logo className="h-8 md:h-12 w-auto" />
              )}
            </Link>
            <small className="type-body-xs text-muted-foreground hidden md:block">
              {copyright ?? `© ${new Date().getFullYear()} All rights reserved`}
            </small>
          </div>

          {/* Right side: Social + Copyright (Mobile) */}
          <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-end gap-4 md:gap-6 w-full md:w-auto">
            <small className="type-body-xs text-muted-foreground md:hidden">
              {copyright ?? `© ${new Date().getFullYear()} All rights reserved`}
            </small>
